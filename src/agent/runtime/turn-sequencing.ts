...
function stripModelControlTokens(text: string) {
  if (!text) return "";
...
function toolSequenceUserIntent(text: string): boolean {
  return TOOL_SEQUENCE_USER_INTENT_RE.test(String(text || "").toLowerCase());
}

export function extractExactResponseTokens(input: string) {
...
function normalizeExactToken(value: string) {
  return String(value || "").replace(/[_-]/g, "");
}

export function repairExactResponseText(input: string, visible: string) {
...
function estimateTaskStepsFromInput(input: string) {
  ...
  if (toolSequenceUserIntent(text) || /\b(all tools|one by one|systematically|without stopping)\b/.test(text)) {
...
export function estimateTaskComplexity(input: string) {
...
export function isPlanningModePrompt(input: string) {
  return /invoked \*\*planning mode\*\* via `\/plan`/i.test(String(input || ""));
}

export function extractPlanningGoalFromPrompt(content: string) {
...
function extractPlanFilePath(text: string) {
  const s = String(text || "");
...
export function shouldSuppressActionPlanAutoContinue(currentRaw: string, previousRaw: string) {
  ...
  if (toolSequenceUserIntent(current)) return false;
