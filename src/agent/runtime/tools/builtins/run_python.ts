...
export default defineTool({
  ...
  emoji: "🐍",
  description:
    "Run Python in browser-only Nodebox via lazy Pyodide (v0.29.4). **Prefer over `run_shell python3`.** Agent one-off REST → `web_fetch`/`web_post`; HTTP inside scripts → `import webagent.http as http`. Supports `code` or workspace `path`, optional `args`, `cwd`, `env`, `packages`, `timeout_ms`. **Create ZIP:** stdlib `zipfile` (no `create_archive` tool). Auto-loads common Pyodide wheels. Hard-blocked: host binaries/subprocess, Playwright, raw DB sockets. Examples: " +
