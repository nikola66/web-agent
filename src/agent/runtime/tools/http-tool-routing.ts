...
export type HttpToolKind = "get" | "post" | "graphql" | "unknown_http";

export type HttpIntentDetection = {
  detected: boolean;
  kind: HttpToolKind;
  suggested_tool: "web_fetch" | "web_post";
...
function pickSuggestedTool(kind: HttpToolKind): "web_fetch" | "web_post" {
  return kind === "post" || kind === "graphql" ? "web_post" : "web_fetch";
}

function defaultExampleArgs(tool: "web_fetch" | "web_post"): Record<string, unknown> {
  if (tool === "web_post") {
    return {
...
export function detectHttpIntentInShellCommand(command: string): HttpIntentDetection {
  ...
  if (GRAPHQL_MARKERS.test(haystack)) kind = "graphql";
