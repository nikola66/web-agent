...
async function writeToolStartTranscript(
  ...
  toolCatalog: Record<string, { emoji?: string } | undefined> | undefined
) {
  const emoji = String(toolCatalog?.[String(name || "")]?.emoji || "").trim();
  const event = createToolStartTranscriptEvent({ name, argsPreview, argsPreviewTruncated });
  process.stdout.write(dim(`${formatToolStartTranscript({ ...event, emoji })}\n`));
...
async function writeToolResultTranscript(
  ...
  const line = formatToolResultTranscript({ ...event, emoji });
...
async function finishToolCallSuccess(
  ...
  await writeToolResultTranscript(prepared.ctx, { name: prepared.name, status: "ok" }, toolCatalog);
