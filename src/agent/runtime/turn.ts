...
export async function agentTurn(
  ...
  const toolHint =
    ...
    "\n\nWiki vault discipline: use `wiki_setup` to create the PARA + Obsidian scaffold (default `knowledge-vault/`). Use `wiki_sync` to push runtime facts/session/learnings into `Resources/KnowledgeVault/` after setup. Use `wiki_search` when the user asks to search the vault or when `memory_search`/`memory_recall` are insufficient. Natural-language equivalents for `/wiki-setup`, `/wiki-sync`, `/wiki-search` should trigger the same tools." +
