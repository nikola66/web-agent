/** Runtime copy — keep in sync with src/core/artifact-preview.ts */
...
export function inferArtifactKind(filename) {
...
export function mimeForArtifactKind(kind, filename) {
  const ext = fileExtension(filename);
  const MIME_BY_EXT = {
