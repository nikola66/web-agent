...
**`scoring_unavailable`** means every recovery attempt in the Loop Guard worker failed (not that Loop Guard is disabled). Scoring runs in a dedicated Web Worker with serialized requests, per-label forwards (lower WASM memory than batched multi-label), premise tail budgets (2000 → 1200 → 800 chars), and worker restart only after repeated identical ONNX failures. Typical root causes: ORT WASM assets missing (`/transformers-ort/`), model weights missing (`public/models/loop-guard/`), **gzip/brotli on `.onnx` or `.wasm`** (production `Caddyfile` serves both uncompressed), or ONNX Runtime Web instability (opaque numeric codes). Verify Network: `ort-wasm-simd-threaded.jsep.wasm` and `model_quantized.onnx` return 200 with no `Content-Encoding`; refresh weights with `npm run download:loop-guard-model`.
