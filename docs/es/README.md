<!-- i18n-sync: en@8293e87 2026-05-20 -->

**Idiomas:** [English](../README.md) · [简体中文](../zh-CN/README.md) · [Español](README.md) · [العربية](../ar/README.md)

# Documentación

Documentación pública para contribuidores y operadores de Web Agent.

**El inglés es la versión canónica.** Las traducciones pueden ir retrasadas; véase [TRANSLATING.md](../TRANSLATING.md).

## Español

| Doc | Descripción |
| --- | --- |
| [agent-notes.md](agent-notes.md) | WebContainer, tool guardrails, persistencia, investigación, bucle de auto-mejora |
| [ARCHITECTURE.md](ARCHITECTURE.md) | Mapa del sistema, IPC, almacenamiento, bucle del agente |
| [testing-checklist.md](testing-checklist.md) | Lista de regresión manual |
| [test-prompts.md](test-prompts.md) | Prompts de humo (instrucciones en español, prompts en inglés) |
| [CAPABILITIES.md](CAPABILITIES.md) | Capacidades modulares |
| [DESIGN.md](DESIGN.md) | Sistema de diseño |
| [AGENTS.md](AGENTS.md) | Reglas para agentes de código IA |
| [SUPPORT.md](SUPPORT.md) | Soporte |
| [CODE_OF_CONDUCT.md](CODE_OF_CONDUCT.md) | Código de conducta |

## Raíz del repositorio

- [README.es.md](../../README.es.md)
- [CONTRIBUTING.es.md](../../CONTRIBUTING.es.md)
- [SECURITY.es.md](../../SECURITY.es.md)

Inglés canónico: [../README.md](../README.md)
